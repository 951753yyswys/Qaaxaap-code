...
struct treap
{
    ...
    vector<tree> t;
    inline long long init(long long num){++cnt;t.push_back({num,0,0,rand(),1});return cnt;}
...
public: 
    ...
    vector<node> t;
    segment_tree(){}
    segment_tree(int n){t.resize(n*4+5);}
    //segment_tree*(int n){t.resize(n*4+5);}
    ...
    inline void init(long long siz,long long m)
    {
        //for(int i=1;i<=4*siz+5;siz++) t.push_back({0,0,0,0});
        mod=m;build(1,1,siz);
    }
