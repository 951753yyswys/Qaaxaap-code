...
namespace yys_wys
{
    ...
    void work()
    {
        ...
        while(t--)
        {
            ...
            for(auto &tmp:y) cin>>tmp;
            for(auto &tmp:c) cin>>tmp,sumc+=tmp;
            vector<int> idx(n);
            int _=-1;
            for(auto &tmp:idx) tmp=++_;
            set<int> waits;
            auto cmp=[&](int i,int j)->bool
            {return x[i]<x[j];};
            sort(idx.begin(),idx.end(),cmp);
            for(int i=0;i<min(3ll,n);i++) waits.insert(idx[i]);
            for(int i=max(0ll,n-3);i<n;i++) waits.insert(idx[i]);
            auto cmp1=[&](int i,int j)->bool
            {return y[i]<y[j];};
            sort(idx.begin(),idx.end(),cmp1);
            for(int i=0;i<min(3ll,n);i++) waits.insert(idx[i]);
            for(int i=max(0ll,n-3);i<n;i++) waits.insert(idx[i]);
            vector<int> waitv(waits.begin(),waits.end());
            int m=waitv.size();
            int ans=-1e18;
            for(int mask=1;mask<(1<<m);mask++) 
            {
                ...
                if (cnt > 4) continue;
                int xmin = 1e18, xmax = -1e18;
                int ymin = 1e18, ymax = -1e18;
                int suml=0;
                for(int i=0;i<m;i++) 
                {
                    if(mask&(1<<i)) 
                ...
                int f_val = 2 * (xmax - xmin + ymax - ymin) - suml;
                if (f_val > ans) {
                    ans = f_val;
                }
            }
