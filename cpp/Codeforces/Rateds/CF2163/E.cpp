    ...
    void work()
    {
        ...
        if(s=="first") 
        {
            ...
            while(t--) 
            {
                ...
                for(int i=0;i<n;i++) for(int j=0;j<n;j++) col[i]+=mp[i][j]-'0';
                for(int j=0;j<n;j++) for(int i=0;i<n;i++) row[j]+=mp[i][j]-'0';
                if(c==1)
                {
        ...
        else 
        {
            ...
            while(t--) 
            {
                ...
                cin>>row>>col;
                int sum=0;
                for(auto tmp:row) sum+=tmp-'0';
                for(auto tmp:col) sum+=tmp-'0';
                if(sum%2) cout<<"0\n";
                else cout<<"1\n";
            }
        }
