...
namespace yys_wys
{
    ...
    void work()
    {
        ...
        while(t--) 
        {
            ...
            for(auto &tmp:a1) cin>>tmp;
            for(auto &tmp:a2) cin>>tmp;
            vector<int> hmin1(n), hmax1(n);
            hmin1[0]=a1[0];
            hmax1[0]=a1[0];
            for(int i=1;i<n;i++) 
            {
                hmin1[i]=min(hmin1[i-1],a1[i]);
                hmax1[i]=max(hmax1[i-1],a1[i]);
            }
            vector<int> dmin2(n),dmax2(n);
            dmin2[n-1]=a2[n-1];
            dmax2[n-1]=a2[n-1];
            for(int i=n-2;i>=0;i--) 
            {
                dmin2[i]=min(dmin2[i+1],a2[i]);
                dmax2[i]=max(dmax2[i+1],a2[i]);
            }
            int maxx=2*n;
            vector<vector<int>> zh(maxx+1);
            for(int j=0;j<n;j++) 
            {
                int mi=min(hmin1[j],dmin2[j]);
                int mx=max(hmax1[j],dmax2[j]);
                if(mi<=maxx) zh[mi].push_back(mx);
            }
            priority_queue<int,vector<int>,greater<int>> pq;
            int ans=0;
            for(int l=maxx;l>=1;l--) 
            {
                for(auto m:zh[l])
                    pq.push(m);
                if(!pq.empty()) 
