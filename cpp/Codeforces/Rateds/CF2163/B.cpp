...
namespace yys_wys
{
    ...
    void work()
    {
        ...
        while(t--) 
        {
            ...
            vector<int> s;
            for(int i=1;i<=n;i++) if(x[i-1]=='1') s.push_back(i);
            if(s.empty()) 
            {
                cout<<0<<endl;
                continue;
            ...
            opt.push_back({A,B});
            vector<int> ls;
            for(auto i:s) if(i<A) ls.push_back(i);
            if(!ls.empty()) 
            {
                int minp=INT_MAX;
                int maxp=INT_MIN;
                ...
                if (!found) 
                {
                    ...
                    opt.push_back({l1, maxi});
                    opt.push_back({l2, mini});
                }
            }
            
            vector<int> right_S;
            for (int i : s) 
                if (i > B) right_S.push_back(i);
            if (!right_S.empty()) 
            {
                int min_p_right = INT_MAX;
                int max_p_right = INT_MIN;
                for (int i : right_S) 
                {
                    if (p[i] < min_p_right) min_p_right = p[i];
                    if (p[i] > max_p_right) max_p_right = p[i];
                }
                bool found = false;
                for (int r=B+1; r<=n; r++) 
                {
                ...
                if (!found) 
                {
                    ...
                    opt.push_back({mini, r1});
                    opt.push_back({maxi, r2});
                }
            }
