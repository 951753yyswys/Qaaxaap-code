...
namespace Qaaxaap
{
    ...
    inline void work()
    {
        ...
        while(T--) 
        {
            ...
            {
                M=max(M,tmp);
                if(tmp>0) f.push_back(tmp);
            }
            int ans=1;
            for(auto tmp:f) ans=ans*(1+tmp)%MOD;
            ans=(ans-1+MOD)%MOD;
            vector<int> dp(n+1,0);
            dp[0]=1;
            for(auto tmp:f) for(int s=n;s>=tmp;s--) dp[s]=(dp[s]+dp[s-tmp]*tmp%MOD)%MOD;
            int fd=0;
            for(int s=1;s<M;s++) fd=(fd+dp[s])%MOD;
            cout<<(ans-fd+MOD)%MOD<<endl;
        }
    }
}
