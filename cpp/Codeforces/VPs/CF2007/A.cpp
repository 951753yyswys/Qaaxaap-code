...
namespace Qaaxaap
{
	void work()
	{
		...
		while(t--)
		{
			int n,m;cin>>n>>m;segment_tree_uni<int> a;
			for(int i=1;i<=n;i++){int tmp;cin>>tmp;a.update(i,tmp);}
			for(int i=1;i<=m;i++){char opt;int l,r;cin>>opt>>l>>r;if(opt=='+'){a.update(l,r,1);}else{a.update(l,r,-1);}cout<<a.query_max(1,n)<<' ';}
			cout<<endl;
		}
	}
}
