...
namespace yys_wys
{
    int p=998244353,N=5e5+5;
    int dp[550000][3],w[114514],h[114514],b[114514],d=250050;
    void work()
    {
        int n;
        cin>>n;
        for(int i=1;i<=n;i++) cin>>w[i]>>h[i]>>b[i];
        for(int i=-250000;i<=250000;i++) dp[i+d][0]=-0x7fffffffffffffff;
        dp[0+d][0]=0;
        for(int i=1;i<=n;i++)
        {
            for(int j=-250000;j<=250000;j++) dp[j+d][i%2]=dp[j+d][1^(i%2)];
            for(int j=-250000;j<=250000;j++)
            {
                dp[min(250000+d,j+d+w[i])][i%2]=max(dp[min(250000+d,j+d+w[i])][i%2],dp[j+d][1^(i%2)]+h[i]);
                dp[max(0ll,j+d-w[i])][i%2]=max(dp[max(0ll,j+d-w[i])][i%2],dp[j+d][1^(i%2)]+b[i]);
            }
        }
        int ans=0;
        for(int i=-250000;i<=0;i++) ans=max(dp[i+d][n%2],ans);
        cout<<ans;
    }
};
signed main()
