...
namespace yys_wys
{
    ...
    int dp[11451][505],w[114514],h[114514],b[114514],d=5050;
    void work()
    {
        int n;
        cin>>n;
        for(int i=1;i<=n;i++) cin>>w[i]>>h[i]>>b[i];
        for(int i=-2500;i<=2500;i++) dp[i+d][0]=-0x7fffffff;
        dp[0+d][0]=0;
        for(int i=1;i<=n;i++)
        {
            for(int j=-2500;j<=2500;j++) dp[j+d][i]=dp[j+d][i-1];
            for(int j=-2500;j<=2500;j++)
            {
                dp[j+d+w[i]][i]=max(dp[j+d+w[i]][i],dp[j+d][i-1]+h[i]);
                dp[j+d-w[i]][i]=max(dp[j+d-w[i]][i],dp[j+d][i-1]+b[i]);
            }
        }
        int ans=0;
        for(int i=-2500;i<=0;i++) ans=max(dp[i+d][n],ans);
        cout<<ans;
    }
};
signed main()
