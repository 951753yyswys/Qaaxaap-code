...
namespace yys_wys
{
    ...
    void work()
    {
        ...
        cin>>n>>q;
        vector<int> a(n);
        segment_tree_uni<int> dzd(0,5e5+5),tril(0,5e5+5);
        for(auto &tmp:a) cin>>tmp,dzd.update(tmp,1),tril.update(tmp,tmp);
        while(q--)
        {
            int opt;
            cin>>opt;
            if(opt-1)
            {
                ...
                int ans=0;
                ans+=r*dzd.query_sum(r,5e5+2);
                ans+=l*dzd.query_sum(0,l);
                if(r-l>=2) ans+=tril.query_sum(l+1,r-1);
                cout<<ans<<endl;    
            }
            else 
            {
