...
namespace DS
{
    ...
    //     {
    //         if(t[ro].l==t[ro].r) return void(t[ro].sum=1);
    //         if(t[ro].r-t[ro].l+1==t[ro].sum) return;
    ...
    //         int ans=0;
    //         if(t[ro<<1].r>=l) ans+=query(ro<<1,l,r);
    //         if()
    //     }
    // };
}
...
namespace Qaaxaap
{
    void work()
    {
        int n,q;
        cin>>n>>q;
        segment_tree_uni<int> tril(1,n);
        while(q--)
        {
            int l,r;
            cin>>l>>r;
            tril.update(l,r,1);
            cout<<n-tril.query_sum(1,n)<<endl;
        }
    }
}
signed main()
