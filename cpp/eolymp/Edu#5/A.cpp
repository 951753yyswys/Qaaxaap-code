...
namespace yys_wys
{
    ...
    void work()
    {
        ...
        cin>>a>>b>>c;
        vector<int> ans;
        for(int i=-10000000;i<=10000000;i++) if(a*i*i+b*i+c==0) ans.push_back(i);
        if(ans.size()==0) cout<<"No roots";
        if(ans.size()==1) cout<<"One root: "<<ans[0];
        if(ans.size()==2) cout<<"Two roots: "<<ans[0]<<' '<<ans[1];
    }
};
signed main()
{   
