...
namespace yys_wys
{
    ...
    void work()
    {
        int n,m,T;
        cin>>n>>m>>T;
        vector<AVL<pair<int,int>>> col(n+1);
        AVL<pair<int,int>> row;
        int pulsar=0;
        for(int i=1;i<=n;i++) for(int j=1;j<=m;j++) yk[i][j]=++pulsar;
        for(int i=1;i<=n;i++) for(int j=1;j<m;j++) col[i].add({j,yk[i][j]});
        for(int i=1;i<=n;i++) row.add({i,yk[i][m]});
        while(T--)
        {
            int x,y;
            cin>>x>>y;
            ...
            {
                int id=x/m;
                pair<int,int> qaa,xaap;
                qaa=row.quv(id);
                xaap=row.quv(n);
                cout<<qaa.second<<endl;
                row.del(qaa);
                row.del(xaap);
                swap(qaa.first,xaap.first);
                row.add(qaa);
                row.add(xaap);
                continue;
            }
