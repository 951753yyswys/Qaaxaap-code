...
namespace DS
{
    ...
    private:
        ...
        inline int new_node(T x)
        {
            ...
            {
                ro=++cnt;
                if(ro>=t.size()) t.push_back({0,0,0,0,0,T{}});
            }
            else
            {
                ro=rubbish.front();
    ...
    public:
        ...
        inline int size(){return tree_size;}
        inline void boom()
        {
            t.clear();
...
namespace yys_wys
{
    ...
    void work()
    {
        ...
        for(auto tmp:a)
        {
            ...
            i++;
            t[tmp].add({i,i});
            pair<int,int> pre=t[tmp].pre({i,i});
            if(pre.second==i-1) 
            {
                t[tmp].del(pre);
                t[tmp].del({i,i});
                t[tmp].add({pre.first,i});
                cnt--;
            }
        }
        while(m--)
        {
            ...
            else
            {
                ...
                while(t[x].size())
                {
                    pair<int,int> fro=t[x].quv(1);
                    t[x].del(fro);
                    t[y].add(fro);
                    pair<int,int> pre,suc;
                    pre=t[y].pre(fro);
                    suc=t[y].suc(fro);
                    if(pre.second==fro.first-1)
                    {
                        t[y].del(pre);
                        t[y].del(fro);
                        cnt--;
                        t[y].add({pre.first,fro.second});
                        fro={pre.first,fro.second};
                    }
                    if(fro.second==suc.first-1)
                    {
                        t[y].del(fro);
                        t[y].del(suc);
                        cnt--;
                        t[y].add({fro.first,suc.second});
                    }
                }
                t[x].boom();
            }
