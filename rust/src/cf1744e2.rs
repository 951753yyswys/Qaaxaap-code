...
fn main() 
{
    ...
    for _ in 0..t 
    {
        let a=scanner.next_i64();
        let b=scanner.next_i64();   
        let c=scanner.next_i64();   
        let d=scanner.next_i64();
        let mut va=Vec::<i64>::new();
        let mut vb=Vec::<i64>::new();
        for i in 1..((a as f64).sqrt() as i64)+1
        {
            if a%i==0 
            {
                va.push(i);
                if i!=a/i {va.push(a/i);}
            }
        }
        for i in 1..((b as f64).sqrt() as i64)+1
        {
            if b%i==0 
            {
                vb.push(i);
                if i!=b/i {vb.push(b/i);}
            }
        }
        //println!("va: {:?}",va);
        //println!("vb: {:?}",vb);
        ...
        for &i in &va 
        {
            ...
            {
                //println!("i: {}, j: {}",i,j);
                if (c/(i*j))*i*j>a&&(d/((a*b)/(i*j)))*((a*b)/(i*j))>b 
                {
                    x=(c/(i*j))*i*j;
                    y=(d/((a*b)/(i*j)))*((a*b)/(i*j));
                    break;
